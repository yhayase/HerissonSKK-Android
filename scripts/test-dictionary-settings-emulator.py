#!/usr/bin/env python3
"""専用エミュレーターの独立DBで辞書管理のドラッグ・保存UIを検証します。"""
import argparse
...
def main():
    ...
    test_class = "se.haya.skk.DictionarySettingsUiTest"
    apks = (
    ...
    report_root = root / "app/build/reports" / "dictionary-settings-ui"
    ...
    if not succeeded(result):
        raise SystemExit("辞書管理UI試験 が失敗・スキップ、または完了しませんでした")
    print(f"辞書管理UI試験 結果: {history}")
