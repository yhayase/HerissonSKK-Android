...
class HardwareKeyMapper {
    ...
    fun previewConfigured(event: KeyEvent, state: BasicSkkState, view: BasicSkkView,
        ...
        rules: se.haya.skk.core.romaji.RomanRuleSet,
        internalEmacsEnabled: Boolean = false): Decoded {
        val savedAccent = accent
        return try { decodeConfigured(event, state, view, bindings, emacsEnabled, rules, internalEmacsEnabled) }
    ...
    fun decodeConfigured(event: KeyEvent, state: BasicSkkState, view: BasicSkkView,
        bindings: KeyBindings, emacsEnabled: Boolean,
        rules: se.haya.skk.core.romaji.RomanRuleSet = se.haya.skk.core.romaji.RomanRuleSet.standard,
        internalEmacsEnabled: Boolean = false): Decoded {
        ...
        val emacsBindingEnabled = emacsEnabled || internalEmacsEnabled && ownsInternalEditing(state, view)
        ...
        if (view.deletion != null || state.registrationSaving) {
            ...
            if (special != null || scalar != null) {
                ...
                if (bindings.resolve(gesture, state, view, emacsBindingEnabled) == BasicSkkAction.Cancel) {
                    reset(); return Decoded.Action(BasicSkkAction.Cancel)
                }
        ...
        if (!event.isMetaPressed && !altText) {
            ...
            if (special != null || scalar != null) {
                ...
                if (bindings.resolveCommand(gesture, state, view, emacsBindingEnabled) == SkkCommand.QUOTE_NEXT) {
        ...
        if (!altText && (event.isCtrlPressed || event.isAltPressed)) {
            ...
            if (text != null || special != null) {
                val action = bindings.resolve(KeyGesture(if (special == null) text else null, special, ctrl = event.isCtrlPressed,
                    alt = event.isAltPressed, shift = event.isShiftPressed), state, view, emacsBindingEnabled)
                if (action != null) { reset(); return Decoded.Action(action) }
            }
            if (emacsBindingEnabled && event.isCtrlPressed && !event.isAltPressed && !event.isShiftPressed) {
        ...
        if (special != null && accent == 0 && !altText) {
            bindings.resolve(KeyGesture(special = special, shift = event.isShiftPressed), state, view, emacsBindingEnabled)?.let {
            ...
            if (emacsBindingEnabled && special in listOf(SpecialKey.PAGE_UP, SpecialKey.PAGE_DOWN)) {
                reset()
        ...
        if (state.mode == InputMode.DIRECT && !altText && accent == 0 && unicode != 0 &&
            unicode and KeyCharacterMap.COMBINING_ACCENT == 0 && !Character.isISOControl(unicode)) {
            bindings.resolve(KeyGesture(String(Character.toChars(unicode)), shift = event.isShiftPressed), state, view, emacsBindingEnabled)?.let {
        ...
        val command = if (!altText && unicode and KeyCharacterMap.COMBINING_ACCENT == 0 &&
            text.codePointCount(0, text.length) == 1) {
            bindings.resolve(KeyGesture(text, shift = event.isShiftPressed), state, view, emacsBindingEnabled)
        } else null
        // n+l は n の終端確定後にモード切替、zl は矢印入力として扱います。
        val readingModeSwitch = state.phase == InputPhase.READING &&
            command in listOf(BasicSkkAction.ToDirect, BasicSkkAction.ToFullwidth) &&
            rules.retainsFollowingKey(state.pendingRomaji, text)
        if (rules.continues(state.pendingRomaji, text) && !readingModeSwitch) {
            return Decoded.Action(BasicSkkAction.Text(text, interpretCommands = false))
        }
        command?.let { return Decoded.Action(it) }
        return Decoded.Action(BasicSkkAction.Text(text, interpretCommands = false))
    }
    ...
    /** Emacs 専用キーを入力先へ越境させず、IME が所有する編集状態だけで受け付けます。 */
    private fun ownsInternalEditing(state: BasicSkkState, view: BasicSkkView): Boolean =
        state.phase != InputPhase.IDLE || state.pendingRomaji.isNotEmpty() ||
            state.registrationDepth > 0 || state.registrationSaving || view.registration != null ||
            view.deletion != null
