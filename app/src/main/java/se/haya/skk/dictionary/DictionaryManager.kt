...
import se.haya.skk.core.dictionary.CompleteDictionaryBackupError
import se.haya.skk.core.dictionary.CompleteDictionaryBackupException
...
class DictionaryManager(
    ...
    override fun complete(query: se.haya.skk.core.CompletionQuery): List<String> =
        readForCaller { currentDictionary().complete(query) }

    override fun predict(query: PredictionQuery): PredictionSearchResult =
        readForCaller { currentDictionary().predict(query) }

    override fun predictionUsage(target: PredictionHistoryTarget): Long? =
        readForCaller { currentDictionary().predictionUsage(target) }
    ...
    ) = savePersonalCandidate(key, candidate, originAllowsSaving, captureInputWriteContext(), null, callback)

    fun saveConfirmedCandidate(
        key: String,
        candidate: SkkDictionaryCandidate,
        ...
        callback: (PersonalWriteResult) -> Unit,
    ) = savePersonalCandidate(key, candidate, originAllowsSaving, inputContext, historyTarget, callback)
    ...
    fun savePersonalCandidate(
        ...
        serialExecutor.execute {
            val saved = runCatching {
                ...
                repository.promotePersonalCandidate(key, candidate, historyTarget) {
                    requireInputWrite(inputContext)
    ...
    internal fun <T> readBlocking(block: () -> T): T {
        val scope = se.haya.skk.core.dictionary.DictionaryReadScope()
        try {
            repeat(256) {
                try { return scope.run(block) } catch (pending: se.haya.skk.core.dictionary.DeferredDictionaryReadException) {
                    pending.load()
                }
            }
