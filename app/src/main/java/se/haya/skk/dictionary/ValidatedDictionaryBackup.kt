        ...
        private fun prepareOpen(context: Context, stream: InputStream, codec: CompleteDictionaryBackupCodec): ValidatedDictionaryBackup {
            ...
            try {
                ...
                val summary = SQLiteDatabase.openOrCreateDatabase(file, null).use { db ->
                    ...
                    db.execSQL("CREATE TABLE usages(reading TEXT NOT NULL, template TEXT NOT NULL, okuri TEXT NOT NULL, committed TEXT NOT NULL, sequence INTEGER NOT NULL, PRIMARY KEY(reading,template,okuri,committed), UNIQUE(sequence))")
                    db.beginTransaction()
                    try {
                        var recordNumber = 0L
                                ...
                                when (record) {
                                    is BackupRecord.Source -> {
                                        ...
                                        sources += DictionarySourceInfo(
                                            record.id,
                                            record.name,
                                            if (kind == 0) DictionarySourceKind.PERSONAL else DictionarySourceKind.SYSTEM,
                                            record.generation,
                                            record.enabled,
                                            record.order,
                                            record.originUrl,
                                    ...
                                    is BackupRecord.Usage -> db.execSQL("INSERT INTO usages VALUES(?,?,?,?,?)", arrayOf<Any?>(record.readingKey, record.templateText, record.okuriCondition ?: "", record.committedText, record.lastUsedSequence))
                                    else -> Unit
                                }
