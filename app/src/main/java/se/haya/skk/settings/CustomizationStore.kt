...
private object CustomizationJson {
    ...
    private val ROOT_FIELDS = setOf(
        ...
        "candidateDisplay", "emacsEnabled", "keyBindings", "internalEmacsEnabled",
    )
    ...
    fun encode(settings: CustomizationSettings): ByteArray {
        ...
        val root = JSONObject().apply {
            ...
            put("internalEmacsEnabled", settings.internalEmacsEnabled)
        }
    ...
    fun decode(bytes: ByteArray): CustomizationSettings {
        ...
        root.requireFields(when (version) {
            1L -> ROOT_FIELDS - setOf("keyBindings", "internalEmacsEnabled")
            in 2L..4L -> ROOT_FIELDS - "internalEmacsEnabled"
            else -> ROOT_FIELDS
        })
        ...
        if (ruleArray.length() > se.haya.skk.core.romaji.RomanRuleSet.MAX_RULES) {
            throw InvalidCustomizationFileException()
        }
        ...
        return CustomizationSettings(
            ...
            bindings,
            if (version < 5) false else root.requiredBoolean("internalEmacsEnabled"),
        )
    }
