...
class CustomizationStoreTest {
    @Test fun `Emacs編集範囲は互換フラグを読み取りモード指定では正規化する`() {
        assertEquals(EmacsEditingMode.DISABLED, CustomizationSettings(0).emacsEditingMode)
        assertEquals(EmacsEditingMode.IME_ONLY,
            CustomizationSettings(0, internalEmacsEnabled = true).emacsEditingMode)
        assertEquals(EmacsEditingMode.IME_AND_APP,
            CustomizationSettings(0, emacsEnabled = true, internalEmacsEnabled = true).emacsEditingMode)

        val settings = CustomizationSettings(0, emacsEnabled = true, internalEmacsEnabled = true)
        val disabled = settings.withEmacsEditingMode(EmacsEditingMode.DISABLED)
        val imeOnly = settings.withEmacsEditingMode(EmacsEditingMode.IME_ONLY)
        val all = settings.withEmacsEditingMode(EmacsEditingMode.IME_AND_APP)
        assertFalse(disabled.emacsEnabled)
        assertFalse(disabled.internalEmacsEnabled)
        assertFalse(imeOnly.emacsEnabled)
        assertTrue(imeOnly.internalEmacsEnabled)
        assertTrue(all.emacsEnabled)
        assertFalse(all.internalEmacsEnabled)
    }

    @Test fun `Emacs編集範囲の各選択は保存後も保持する`() {
        val path = temporaryPath()
        val direct = Executor { it.run() }
        EmacsEditingMode.entries.forEach { mode ->
            CustomizationStore(path, direct, direct).use { store ->
                store.loadAsync()
                store.save(store.snapshot.withEmacsEditingMode(mode), store.snapshot.generation) {
                    assertTrue(it is CustomizationWriteResult.Applied)
                }
            }
            CustomizationStore(path, direct, direct).use { store ->
                store.loadAsync()
                assertEquals(mode, store.snapshot.emacsEditingMode)
            }
        }
    ...
    @Test fun `AZIKとカスタムキーを版3で保存し旧版は標準割当として読む`() {
        ...
        assertEquals(se.haya.skk.core.keys.KeyBindings(), store.snapshot.keyBindings)
        ...
        assertEquals(5, json.getInt("documentVersion"))
        store.close()
    ...
    @Test fun `版2のカスタムキーを保持し衝突する新操作は未割当で移行する`() {
        val path = temporaryPath()
        val direct = Executor { it.run() }
        val oldBindings = se.haya.skk.core.keys.KeyBindings.defaults
            .filterKeys { it !in se.haya.skk.core.keys.KeyBindings.optionalCommands }.toMutableMap()
        oldBindings[se.haya.skk.core.keys.SkkCommand.HALFWIDTH] =
            se.haya.skk.core.keys.KeyGesture("q", ctrl = true)
        oldBindings[se.haya.skk.core.keys.SkkCommand.EDIT_LEFT] =
            se.haya.skk.core.keys.KeyGesture("v", ctrl = true)
        val initial = CustomizationSettings(0, emacsEnabled = true,
            keyBindings = se.haya.skk.core.keys.KeyBindings(oldBindings))
        CustomizationStore(path, direct, direct).use { store ->
            store.loadAsync(); store.save(initial, 0) {}
        }
        val legacy = org.json.JSONObject(path.readText()).apply {
            put("documentVersion", 2)
            remove("internalEmacsEnabled")
        ...
        CustomizationStore(path, direct, direct).use { store ->
            ...
            assertEquals(oldBindings.getValue(se.haya.skk.core.keys.SkkCommand.EDIT_LEFT),
                store.snapshot.keyBindings.bindings[se.haya.skk.core.keys.SkkCommand.EDIT_LEFT])
            org.junit.Assert.assertFalse(store.snapshot.keyBindings.bindings.containsKey(
                se.haya.skk.core.keys.SkkCommand.EDIT_PAGE_DOWN))
    ...
    @Test fun `内部 Emacs 設定を保存し旧版では無効として読む`() {
        val path = temporaryPath()
        val direct = Executor { it.run() }
        CustomizationStore(path, direct, direct).use { store ->
            store.loadAsync()
            store.save(CustomizationSettings(0, internalEmacsEnabled = true), 0) {
                assertTrue(it is CustomizationWriteResult.Applied)
            }
        }
        val current = org.json.JSONObject(path.readText())
        assertEquals(true, current.getBoolean("internalEmacsEnabled"))
        current.put("documentVersion", 4)
        current.remove("internalEmacsEnabled")
        path.writeText(current.toString())
        CustomizationStore(path, direct, direct).use { store ->
            store.loadAsync()
            assertEquals(false, store.snapshot.internalEmacsEnabled)
        }
        deleteAtomicFiles(path)
    }
    ...
    @Test fun `追加規則の先頭文字と有効な編集割当の競合を拒否する`() {
        ...
        val moved = se.haya.skk.core.keys.KeyBindings(se.haya.skk.core.keys.KeyBindings.defaults +
            (se.haya.skk.core.keys.SkkCommand.TOGGLE_KANA to se.haya.skk.core.keys.KeyGesture("[")))
        CustomizationSettings(0, CustomizationProfile.CUSTOM, listOf(RomajiRule("q", "ん")), keyBindings = moved)
        val collision = se.haya.skk.core.keys.KeyBindings(se.haya.skk.core.keys.KeyBindings.defaults +
            (se.haya.skk.core.keys.SkkCommand.EDIT_HOME to se.haya.skk.core.keys.KeyGesture("j", ctrl = true)))
        CustomizationSettings(0, keyBindings = collision)
        assertThrows(IllegalArgumentException::class.java) {
            CustomizationSettings(0, emacsEnabled = true, keyBindings = collision)
        }
        assertThrows(IllegalArgumentException::class.java) {
            CustomizationSettings(0, internalEmacsEnabled = true, keyBindings = collision)
        }
    }
    ...
    @Test fun `設定モデルは規則を防御コピーしプロファイル境界を検証する`() {
        ...
        CustomizationSettings(0, CustomizationProfile.CUSTOM,
            se.haya.skk.core.romaji.Romanizer.standardRules)
        assertThrows(IllegalArgumentException::class.java) {
    ...
    private fun custom(
        ...
        candidate,
        emacs,
        internalEmacsEnabled = internalEmacs,
    )
