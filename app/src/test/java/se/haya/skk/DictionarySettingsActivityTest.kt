...
class DictionarySettingsActivityTest {
    ...
    @Test
    fun `辞書行は利用状態を要約に含める`() {
        assertEquals("有効　優先順位 2　利用状態を確認できません", dictionarySourceSummary(
            enabled = true,
            index = 1,
            availability = "利用状態を確認できません",
        ))
    }

    @Test
    fun `公式辞書の副操作にも利用者向け名称を使う`() {
        val source = DictionarySourceInfo(
            "official-skk-S", "SKK-JISYO.S", DictionarySourceKind.SYSTEM, 1, true, 0,
        )

        assertEquals("SKK-JISYO.S", dictionarySourceDisplayName(source))
    }
