...
class CompleteBackupIoActivityTest {
    ...
    @Test fun `出力の書込失敗もclose失敗も成功表示にしない`() {
        for (failDuringWrite in listOf(true, false)) {
            ...
            choose(CompleteDictionaryBackupFragment.KEY_EXPORT)
    ...
    @Test fun `復元元の読取失敗とclose失敗では確認画面へ進まない`() {
        ...
        for (failDuringRead in listOf(true, false)) {
            ...
            choose(CompleteDictionaryBackupFragment.KEY_RESTORE)
    ...
    private fun action(key: String): Preference =
    ...
    private fun status() = activity.findViewById<TextView>(R.id.settings_status).text.toString()
