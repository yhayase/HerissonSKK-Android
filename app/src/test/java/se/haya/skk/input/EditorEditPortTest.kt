...
class EditorEditPortTest {
    ...
    private class Connection(var text: String, var start: Int, var end: Int = start) :
        ...
        var afterCursorCap = Int.MAX_VALUE
        ...
        override fun getTextAfterCursor(length: Int, flags: Int): CharSequence? {
            ...
            val finish = (end + minOf(length, afterCursorCap)).coerceAtMost(text.length)
            return text.substring(end, finish)
        }
    ...
    private class Harness(text: String, cursor: Int, anchor: Int = cursor,
        copyKilledText: (String) -> Boolean = { true },
        ownsKilledText: (String) -> Boolean = { true }) {
    ...
    @Test fun `ネイティブ移動後のCkは遅延選択通知まで削除を待つ`() {
        val copied = ArrayList<String>()
        val h = Harness("abc\ndef", 7, copyKilledText = { copied += it; true })
        assertEquals(EditorEditResult.Outcome.NATIVE_ISSUED, h.run(EditCommand.LEFT).outcome)
        assertEquals(6, h.connection.start)
        h.submit(EditCommand.KILL_LINE)
        h.worker.run()
        assertEquals(0, h.connection.deleteCalls)
        assertTrue(copied.isEmpty())
        h.update { it.copy(selectionStart = 6, selectionEnd = 6) }
        h.port.acknowledgeNativeSelection()
        h.worker.run()
        h.callbacks.run()
        assertEquals(EditorEditResult.Outcome.APPLIED, h.results.last().outcome)
        assertEquals("abc\nde", h.connection.text)
        assertEquals(listOf("f"), copied)
    }

    @Test fun `連続ネイティブ移動の途中通知では座標を推測してCkを実行しない`() {
        val copied = ArrayList<String>()
        val h = Harness("abcdef", 6, copyKilledText = { copied += it; true })
        h.run(EditCommand.LEFT)
        h.run(EditCommand.LEFT)
        assertEquals(4, h.connection.start)
        h.submit(EditCommand.KILL_LINE)
        h.update { it.copy(selectionStart = 5, selectionEnd = 5) }
        h.port.acknowledgeNativeSelection()
        h.worker.run()
        h.callbacks.run()
        assertEquals(EditorEditResult.Outcome.UNSUPPORTED, h.results.last().outcome)
        h.update { it.copy(selectionStart = 4, selectionEnd = 4) }
        h.port.acknowledgeNativeSelection()
        h.worker.run()
        h.callbacks.run()
        assertEquals("abcdef", h.connection.text)
        assertTrue(copied.isEmpty())
        assertEquals(0, h.connection.deleteCalls)
    }

    @Test fun `ネイティブ移動が端で変化しない場合は期限後に一度だけ文末を照合する`() {
        val h = Harness("abc", 3)
        h.run(EditCommand.RIGHT)
        h.submit(EditCommand.KILL_LINE)
        h.now = 500L
        h.timer.fire()
        h.worker.run()
        h.callbacks.run()
        assertEquals(EditorEditResult.Reason.END_OF_BUFFER, h.results.last().reason)
        assertEquals(0, h.connection.deleteCalls)
        assertTrue(h.timer.tasks.isEmpty())
        val completed = h.results.size
        h.submit(EditCommand.KILL_LINE)
        h.worker.run()
        h.callbacks.run()
        assertEquals(completed + 1, h.results.size)
        assertEquals(EditorEditResult.Reason.END_OF_BUFFER, h.results.last().reason)
        assertTrue(h.timer.tasks.isEmpty())
    }

    @Test fun `ネイティブ待機期限後も座標不一致なら推測せずCkを拒否する`() {
        val h = Harness("abc", 3)
        h.run(EditCommand.LEFT)
        h.submit(EditCommand.KILL_LINE)
        h.now = 500L
    ...
    @Test fun `ネイティブ選択待機中の外部変更と終了はCkを失効させる`() {
        for (closed in listOf(true, false)) {
            val h = Harness("abc", 3)
            h.run(EditCommand.LEFT)
            h.submit(EditCommand.KILL_LINE)
            if (closed) h.port.close() else h.update { it.copy(revision = 1) }
            h.now = 500L
            h.timer.fire()
            h.worker.run()
            h.callbacks.run()
    ...
    @Test fun `連続Ckは改行を含めて累積し文末では保持する`() {
        val copied = ArrayList<String>()
        val h = Harness("abc\ndef", 1, copyKilledText = { copied += it; true })
        repeat(3) { assertEquals(EditorEditResult.Outcome.APPLIED, h.run(EditCommand.KILL_LINE).outcome) }
        assertEquals(listOf("bc", "bc\n", "bc\ndef"), copied)
        assertEquals("a", h.connection.text)
        assertEquals(EditorEditResult.Reason.END_OF_BUFFER, h.run(EditCommand.KILL_LINE).reason)
        assertEquals(3, copied.size)
    }

    @Test fun `外部選択と非kill操作は累積を打ち切る`() {
        for (external in listOf(true, false)) {
            val copied = ArrayList<String>()
            val h = Harness("abc\ndef", 1, copyKilledText = { copied += it; true })
            h.run(EditCommand.KILL_LINE)
            if (external) h.update { it.copy(revision = it.revision + 1) }
            else h.run(EditCommand.COPY)
            h.run(EditCommand.KILL_LINE)
            assertEquals(listOf("bc", "\n"), copied)
        }
    }

    @Test fun `クリップボード所有権を失うと既存内容を追記しない`() {
        val copied = ArrayList<String>()
        val h = Harness("abc\ndef", 1, copyKilledText = { copied += it; true },
            ownsKilledText = { false })
        h.run(EditCommand.KILL_LINE)
        h.run(EditCommand.KILL_LINE)
        assertEquals(listOf("bc", "\n"), copied)
    }

    @Test fun `kill中の別操作は遅延完了後にも累積を再開させない`() {
        val copied = ArrayList<String>()
        val h = Harness("abc\ndef", 1, copyKilledText = { copied += it; true })
        h.connection.onEdit = { h.port.breakKillChain() }
        h.run(EditCommand.KILL_LINE)
    ...
    @Test fun `所有確認中にkill連鎖が切れたら古い削除文字列を追記しない`() {
        val copied = ArrayList<String>()
        lateinit var h: Harness
        h = Harness("abc\ndef", 1, copyKilledText = { copied += it; true }, ownsKilledText = {
            h.port.breakKillChain()
            true
        })
        ...
        assertEquals(listOf("bc", "\n"), copied)
        assertEquals("adef", h.connection.text)
    }

    @Test fun `所有確認中に操作が無効になったらクリップボードを書き換えない`() {
        for (invalidation in listOf("close", "state", "timeout")) {
            val copied = ArrayList<String>()
            lateinit var h: Harness
            h = Harness("abc\ndef", 1, copyKilledText = { copied += it; true }, ownsKilledText = {
                when (invalidation) {
                    "close" -> h.port.close()
                    "state" -> h.update { state -> state.copy(active = false) }
                    else -> h.now = 500L
                }
                true
            })
            ...
            val result = h.run(EditCommand.KILL_LINE)

            assertEquals(invalidation, listOf("bc"), copied)
            assertEquals(invalidation, "a\ndef", h.connection.text)
            assertEquals(invalidation, if (invalidation == "timeout")
                EditorEditResult.Outcome.FAILED_OR_UNKNOWN else EditorEditResult.Outcome.STALE, result.outcome)
        }
    }

    @Test fun `Ckは最終行をクリップボードへ保存してから削除する`() {
        val copied = ArrayList<String>()
        val h = Harness("abc", 1, copyKilledText = { copied += it; true })
        assertEquals(EditorEditResult.Outcome.APPLIED, h.run(EditCommand.KILL_LINE).outcome)
        assertEquals(listOf("bc"), copied)
        assertEquals("a", h.connection.text)

        val rejected = Harness("abc", 1, copyKilledText = { false })
        assertEquals(EditorEditResult.Outcome.FAILED_OR_UNKNOWN,
            rejected.run(EditCommand.KILL_LINE).outcome)
        assertEquals("abc", rejected.connection.text)
        assertEquals(0, rejected.connection.deleteCalls)

        lateinit var stale: Harness
        stale = Harness("abc", 1, copyKilledText = {
            stale.update { state -> state.copy(active = false) }
            true
        })
        assertEquals(EditorEditResult.Outcome.STALE, stale.run(EditCommand.KILL_LINE).outcome)
        assertEquals("abc", stale.connection.text)
        assertEquals(0, stale.connection.deleteCalls)
    }

    @Test fun `貼り付けと元に戻すはAndroidの標準操作へ一度だけ渡す`() {
        for ((command, id) in listOf(EditCommand.PASTE to android.R.id.paste,
                EditCommand.UNDO to android.R.id.undo)) {
            val h = Harness("abc", 1)
            assertEquals(EditorEditResult.Outcome.NATIVE_ISSUED, h.run(command).outcome)
            assertEquals(listOf(id), h.connection.contextActions)
        }
    }
    ...
    @Test fun `証明できない取得窓末尾を行末や文書末尾として削除しない`() {
        val word = Harness("abc", 1)
        assertEquals(EditorEditResult.Outcome.UNSUPPORTED, word.run(EditCommand.WORD_FORWARD).outcome)
        assertEquals(0, word.connection.edits)
        val truncated = Harness("a".repeat(5_000), 1)
        truncated.connection.windowed = true
        truncated.connection.afterCursorCap = 2_048
        assertEquals(EditorEditResult.Outcome.UNSUPPORTED, truncated.run(EditCommand.KILL_LINE).outcome)
        assertEquals(0, truncated.connection.edits)
        val h = Harness("ab\r", 2)
        assertEquals(EditorEditResult.Outcome.APPLIED, h.run(EditCommand.KILL_LINE).outcome)
        assertEquals("ab", h.connection.text)
        val unavailableProof = Harness("abc", 1)
        unavailableProof.connection.extractedUnavailable = true
        assertEquals(EditorEditResult.Outcome.UNSUPPORTED,
            unavailableProof.run(EditCommand.KILL_LINE).outcome)
    }
