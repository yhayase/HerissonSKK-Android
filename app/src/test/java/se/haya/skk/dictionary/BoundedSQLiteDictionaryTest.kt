...
class BoundedSQLiteDictionaryTest {
    ...
    @Test fun `予測のキャッシュミスは遅延読込し完了空と未判定を混同しない`() {
        val repository = repository()
        repository.importSystem("system", "辞書", document("かな /仮名/\nかなう /叶う/"))
        val manager = DictionaryManager(repository, direct, direct)
        manager.loadAsync()
        assertThrows(DeferredDictionaryReadException::class.java) { manager.predict(PredictionQuery("かな")) }
        val loaded = manager.readBlocking { manager.predict(PredictionQuery("かな")) }
        assertEquals(listOf("仮名", "叶う"), (loaded as PredictionSearchResult.Ready).items.map { it.committedText })
        val reads = repository.readStats.prefixReads
        assertTrue(manager.predict(PredictionQuery("かな")) is PredictionSearchResult.Ready)
        assertEquals(reads, repository.readStats.prefixReads)
        assertEquals(PredictionSearchResult.ConfirmedEmpty,
            manager.readBlocking { manager.predict(PredictionQuery("なし")) })
        manager.close()
    }

    @Test fun `保存済みシステム辞書がなくても許可された代替辞書から予測する`() {
        val repository = repository()
        val fallback = SkkDictionarySource("builtin", 1, document("かな /仮名/").entries)
        val manager = DictionaryManager(repository, direct, direct, fallbackSystems = listOf(fallback))
        manager.loadAsync()

        val result = manager.readBlocking { manager.predict(PredictionQuery("か")) }
            as PredictionSearchResult.Ready
        assertEquals(listOf("仮名"), result.items.map { it.committedText })
        assertEquals("かな", result.items.single().historyTarget.readingKey)
        assertEquals(0, repository.readStats.fullSnapshotReads)
        manager.close()
    }

    @Test fun `一文字予測はUIスレッドで走査せず五万件の資料を文字上限で未判定にする`() {
        val repository = repository()
        repository.importSystem("large", "辞書", document(
            (0..PredictionQuery.MAX_WORK_ITEMS).joinToString("\n") { index ->
                "あ${index.toString().padStart(5, '0')} /候/"
            },
        ))
        val manager = DictionaryManager(repository, direct, direct)
        manager.loadAsync()

        assertThrows(DeferredDictionaryReadException::class.java) {
            manager.predict(PredictionQuery("あ", limit = 7))
        }
        assertEquals(0, repository.readStats.prefixReads)

        val started = System.nanoTime()
        val result = manager.readBlocking { manager.predict(PredictionQuery("あ", limit = 7)) }
        val elapsedMillis = (System.nanoTime() - started) / 1_000_000
        assertEquals(
            PredictionSearchResult.Indeterminate(PredictionSearchFailure.RESULT_LIMIT),
            result,
        )
        assertEquals(1, repository.readStats.prefixReads)
        assertEquals(0, repository.readStats.fullSnapshotReads)
        assertBounded(manager)
        println("one-character prediction: ${PredictionQuery.MAX_WORK_ITEMS + 1} rows, ${elapsedMillis}ms")
        manager.close()
    }

    @Test fun `予測は候補または履歴の巨大な文字列を蓄積する前に未判定にする`() {
        repository().use { candidates ->
            candidates.importSystem("large", "辞書", document(
                "あ /候;${"注".repeat(PredictionQuery.MAX_TOTAL_RESULT_CHARS)}/",
            ))
            assertEquals(
                PredictionSearchResult.Indeterminate(PredictionSearchFailure.RESULT_LIMIT),
                candidates.predict(PredictionQuery("あ"), candidates.dictionaryRevision()),
            )
        }

        repository().use { history ->
            history.promotePersonalCandidate(
                "あ",
                SkkDictionaryCandidate("候"),
                PredictionHistoryTarget(
                    "あ",
                    "候",
                    null,
                    "履".repeat(PredictionQuery.MAX_TOTAL_RESULT_CHARS),
                ),
            )
            assertEquals(
                PredictionSearchResult.Indeterminate(PredictionSearchFailure.RESULT_LIMIT),
                history.predict(PredictionQuery("あ"), history.dictionaryRevision()),
            )
        }
    }
}
