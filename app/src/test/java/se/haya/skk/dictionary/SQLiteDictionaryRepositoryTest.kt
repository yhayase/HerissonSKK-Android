...
class SQLiteDictionaryRepositoryTest {
    ...
    @Test fun `ネットワーク更新元は本文と同じトランザクションで保存し失敗時は旧値を残す`() {
        var fail = false
        SQLiteDictionaryRepository(context, databaseName()) { point ->
            if (fail && point == DictionaryWritePoint.BEFORE_PUBLICATION) error("試験用障害")
        }.use { repository ->
            repository.importSystem("network", "辞書", document("かな /旧/"), originUrl = "https://example.com/old.gz")
            fail = true
            assertThrows(IllegalStateException::class.java) {
                repository.importSystem(
                    ...
                    document("かな /新/"),
                    1,
                    "https://example.com/new.gz",
                )
            }
            val source = repository.listSources().single { it.id == "network" }
            assertEquals("https://example.com/old.gz", source.originUrl)
            assertEquals(listOf("旧"), lookupTexts(repository, "かな"))
        }
    }

    @Test fun `大文字を含む HTTPS スキームのネットワーク更新元を保存できる`() {
        SQLiteDictionaryRepository(context, databaseName()).use { repository ->
            repository.importSystem("network", "辞書", document("かな /内容/"), originUrl = "HTTPS://example.com/SKK-JISYO.gz")

            assertEquals("HTTPS://example.com/SKK-JISYO.gz", repository.listSources().single { it.id == "network" }.originUrl)
        }
    }

    @Test fun `content URI のファイル更新元を本文とともに保存できる`() {
        SQLiteDictionaryRepository(context, databaseName()).use { repository ->
            val origin = "content://com.example.documents/document/SKK-JISYO.user"

            repository.importSystem("file", "SKK-JISYO.user", document("かな /内容/"), originUrl = origin)

            assertEquals(origin, repository.listSources().single { it.id == "file" }.originUrl)
        }
    }

    @Test fun `ファイルとHTTPS以外の更新元を拒否する`() {
        SQLiteDictionaryRepository(context, databaseName()).use { repository ->
            listOf("file:///sdcard/SKK-JISYO", "http://example.com/SKK-JISYO", "content:opaque").forEach { origin ->
                assertThrows(IllegalArgumentException::class.java) {
                    repository.importSystem("invalid", "辞書", document("かな /内容/"), originUrl = origin)
                }
            }
        }
    }
    ...
    @Test fun `個人辞書マージは取込順を先頭にし注釈と送り条件を構造で保持する`() {
        SQLiteDictionaryRepository(context, databaseName()).use { repository ->
            ...
            val nihon = repository.lookup("にほん").personal!!
                .let { se.haya.skk.core.dictionary.CompositeSkkDictionary(it) }
    ...
    @Test fun `確定履歴は通常学習と原子的に保存し読みをまたいで最新順に予測する`() {
        val name = databaseName()
        SQLiteDictionaryRepository(context, name).use { repository ->
            repository.importSystem("system", "辞書", document("かな /共通;かな/仮名/\nかなう /共通;かなう/叶う/"))
            repository.promotePersonalCandidate("かなう", SkkDictionaryCandidate("叶う"),
                PredictionHistoryTarget("かなう", "叶う", null, "叶う"))
            repository.promotePersonalCandidate("かな", SkkDictionaryCandidate("仮名"),
                PredictionHistoryTarget("かな", "仮名", null, "仮名"))
            assertTrue(checkNotNull(repository.predictionUsage(
                PredictionHistoryTarget("かなう", "叶う", null, "叶う"), repository.dictionaryRevision())) > 0)
            val result = repository.predict(PredictionQuery("かな"), repository.dictionaryRevision())
                as PredictionSearchResult.Ready
            assertEquals(listOf(
                PredictionHistoryTarget("かな", "仮名", null, "仮名"),
                PredictionHistoryTarget("かなう", "叶う", null, "叶う"),
                PredictionHistoryTarget("かな", "共通", null, "共通"),
            ), result.items.map { it.historyTarget })
            assertEquals(listOf(2L, 1L, null), result.items.map { it.lastUsedSequence })
            assertEquals(listOf("仮名", "叶う", "共通"), result.items.map { it.committedText })
            assertEquals(listOf("かな", "かなう", "かな"), result.items.map { it.historyTarget.readingKey })
        }
        SQLiteDictionaryRepository(context, name).use { reopened ->
            val target = PredictionHistoryTarget("かな", "仮名", null, "仮名")
            assertTrue(checkNotNull(reopened.predictionUsage(target, reopened.dictionaryRevision())) > 0)
        }
    }

    @Test fun `数値の元テンプレートと確定文字列を別々に履歴へ保存する`() {
        SQLiteDictionaryRepository(context, databaseName()).use { repository ->
            val target = PredictionHistoryTarget("だい#", "第#0", null, "第12")
            repository.promotePersonalCandidate("だい#", SkkDictionaryCandidate("第#0"), target)
            assertTrue(checkNotNull(repository.predictionUsage(target, repository.dictionaryRevision())) > 0)
            assertEquals(null, repository.predictionUsage(target.copy(committedText = "第13"), repository.dictionaryRevision()))
        }
    }
}
