...
sealed interface RomanPredictionExpansion {
    data class Complete(val outputs: List<String>) : RomanPredictionExpansion
    data object Indeterminate : RomanPredictionExpansion
}

/** 検証と索引構築を一度だけ終え、複数の入力状態で再利用できるローマ字規則集合です。 */
class RomanRuleSet private constructor(
    private val byInput: Map<String, RomajiRule>,
    private val byPrefix: Map<String, List<RomajiRule>>,
    ...
    /** n+l など、後続キーを消費せず次の入力の残余に戻す規則かを返します。 */
    fun retainsFollowingKey(pending: String, text: String): Boolean =
        pending.isNotEmpty() && text.length == 1 &&
            exact(pending + text)?.remaining == text.lowercase()

    /** 予測に使えるかなを、実行時と同じ残余規則から有界に列挙します。 */
    fun possibleOutputs(pending: String, limit: Int = 64): RomanPredictionExpansion {
        if (pending.isEmpty() || limit <= 0) return RomanPredictionExpansion.Complete(emptyList())
        val result = linkedSetOf<String>()
        val queue = ArrayDeque<String>()
        val discovered = linkedSetOf(pending)
        val edges = linkedMapOf<String, MutableSet<String>>()
        queue += pending
        var work = 0
        while (queue.isNotEmpty()) {
            val state = queue.removeFirst()
            for (rule in byPrefix[state].orEmpty()) {
                if (++work > MAX_PREDICTION_WORK) return RomanPredictionExpansion.Indeterminate
                val output = rule.terminalOutput ?: rule.output
                if (output.isNotEmpty()) {
                    if (output.length > MAX_PREDICTION_OUTPUT_CHARS) {
                        return RomanPredictionExpansion.Indeterminate
                    }
                    if (result.add(output) && result.size > limit) {
                        return RomanPredictionExpansion.Indeterminate
                    }
                } else if (rule.remaining.isNotEmpty()) {
                    edges.getOrPut(state) { linkedSetOf() } += rule.remaining
                    if (discovered.add(rule.remaining)) queue += rule.remaining
                }
            }
        }
        val indegree = discovered.associateWith { 0 }.toMutableMap()
        edges.values.forEach { targets -> targets.forEach { target -> indegree[target] = indegree.getValue(target) + 1 } }
        val acyclic = ArrayDeque(indegree.filterValues { it == 0 }.keys)
        var resolvedStates = 0
        while (acyclic.isNotEmpty()) {
            val state = acyclic.removeFirst()
            resolvedStates++
            edges[state].orEmpty().forEach { target ->
                indegree[target] = indegree.getValue(target) - 1
                if (indegree.getValue(target) == 0) acyclic += target
            }
        }
        if (resolvedStates != discovered.size) return RomanPredictionExpansion.Indeterminate
        return RomanPredictionExpansion.Complete(result.toList())
    }
    ...
    companion object {
        ...
        const val MAX_PREDICTION_WORK = 50_000
        const val MAX_PREDICTION_OUTPUT_CHARS = 1_024
        ...
        fun compile(rules: Collection<RomajiRule>): RomanRuleSet {
            ...
            val byPrefix = LinkedHashMap<String, MutableList<RomajiRule>>()
            val inputCharacters = LinkedHashSet<Char>()
            byInput.keys.forEach { input ->
                inputCharacters += input.toList()
                for (length in 1..input.length) {
                    val prefix = input.substring(0, length)
                    prefixes += prefix
                    byPrefix.getOrPut(prefix) { mutableListOf() } += byInput.getValue(input)
                }
            ...
            return RomanRuleSet(
                ...
                byPrefix = Collections.unmodifiableMap(byPrefix.mapValues { (_, rules) ->
