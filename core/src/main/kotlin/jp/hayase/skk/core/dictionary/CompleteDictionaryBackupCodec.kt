...
class CompleteDictionaryBackupCodec(
    ...
    fun validate(
        ...
        while (true) {
            ...
            if (next == '\n'.code) {
                val record = decodeRecord(decodeUtf8(line.toByteArray(), validator.nextRecordNumber), validator.nextRecordNumber)
                validator.accept(record)
    ...
    private fun decodeRecord(line: String, recordNumber: Long): BackupRecord {
        ...
        return when (type) {
            ...
            "end" -> {
                fields.requireExactly(recordNumber, "type", "sourceCount", "candidateCount", "suppressionCount", "sourceVersionCount")
                BackupRecord.End(
    ...
    private fun encode(record: BackupRecord): String = buildString {
        append('{')
        when (record) {
...
private class RecordValidator(private val limits: CompleteDictionaryBackupLimits) {
    private enum class Section { START, SOURCES, CANDIDATES, SUPPRESSIONS, VERSIONS, END }
    ...
    private fun acceptHeader(record: BackupRecord.Header) {
        ...
        if (record.version != COMPLETE_DICTIONARY_BACKUP_VERSION) fail(recordNumber, CompleteDictionaryBackupError.UNSUPPORTED_VERSION)
    ...
    private fun acceptEnd(record: BackupRecord.End) {
        ...
        summary = BackupSummary(producerVersion!!, sources.size, candidateCount, suppressionCount, sourceVersionCount)
        section = Section.END
    }
