...
class DeferredDictionaryEngineTest {
    ...
    @Test fun `削除完了の再検索を延期しても保存済み削除を再要求しない`() {
        val candidate = DictionaryCandidate("日本", selection = jp.hayase.skk.core.dictionary.CandidateSelection(
            1, listOf(jp.hayase.skk.core.dictionary.SelectedCandidateOrigin("personal", 1, true,
                "にほん", "日本", null))))
        var defer = false
        var removed = false
