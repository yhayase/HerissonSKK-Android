...
class CompleteDictionaryBackupCodecTest {
    ...
    @Test
    fun `大文字を含む HTTPS スキームの取得元を含むバックアップを受理する`() {
        val records = listOf(
            BackupRecord.Header(COMPLETE_DICTIONARY_BACKUP_FORMAT, COMPLETE_DICTIONARY_BACKUP_VERSION, "test"),
            BackupRecord.Source("personal", "個人", BackupSourceKind.PERSONAL, 0, true, 0),
            BackupRecord.Source("network", "ネットワーク", BackupSourceKind.SYSTEM, 1, true, 0,
                "HTTPS://example.com/SKK-JISYO.gz"),
            BackupRecord.SourceVersion("network", 1),
            BackupRecord.SourceVersion("personal", 0),
            BackupRecord.End(2, 0, 0, 2),
        )
        val output = ByteArrayOutputStream()

        codec.write(records.asSequence(), output)
        val consumed = mutableListOf<BackupRecord>()
        codec.validate(ByteArrayInputStream(output.toByteArray()), consumed::add)

        assertEquals(records, consumed)
    }

    @Test
    fun `content URI の取得元を含むバックアップを保持する`() {
        val origin = "content://com.example.documents/document/SKK-JISYO.user"
        val records = listOf(
            BackupRecord.Header(COMPLETE_DICTIONARY_BACKUP_FORMAT, COMPLETE_DICTIONARY_BACKUP_VERSION, "test"),
            BackupRecord.Source("personal", "個人", BackupSourceKind.PERSONAL, 0, true, 0),
            BackupRecord.Source("file", "SKK-JISYO.user", BackupSourceKind.SYSTEM, 1, true, 0, origin),
            BackupRecord.SourceVersion("file", 1),
            BackupRecord.SourceVersion("personal", 0),
            BackupRecord.End(2, 0, 0, 2),
        )
        val output = ByteArrayOutputStream()
    ...
    fun `未知フィールド 重複フィールド 型違い 未知版を分類して拒否する`() {
        val invalid = listOf(
            ...
            "{\"type\":\"header\",\"format\":\"$COMPLETE_DICTIONARY_BACKUP_FORMAT\",\"version\":4,\"producerVersion\":\"x\"}\n" to CompleteDictionaryBackupError.UNSUPPORTED_VERSION,
        )
    ...
    @Test fun `v3は使用履歴を往復しv2は履歴なしとして読む`() {
        val records = listOf(
            BackupRecord.Header(COMPLETE_DICTIONARY_BACKUP_FORMAT, 3, "test"),
            BackupRecord.Source("personal", "個人", BackupSourceKind.PERSONAL, 0, true, 0),
            BackupRecord.SourceVersion("personal", 0),
            BackupRecord.Usage("だい#", "第#0", null, "第12", 7),
            BackupRecord.End(1, 0, 0, 1, 1),
        )
        val output = ByteArrayOutputStream()
        val summary = codec.write(records.asSequence(), output)
        val decoded = mutableListOf<BackupRecord>()
        assertEquals(summary, codec.validate(ByteArrayInputStream(output.toByteArray()), decoded::add))
        assertEquals(records, decoded)

        val old = listOf(
            BackupRecord.Header(COMPLETE_DICTIONARY_BACKUP_FORMAT, 2, "test"),
            BackupRecord.Source("personal", "個人", BackupSourceKind.PERSONAL, 0, true, 0),
            BackupRecord.SourceVersion("personal", 0),
            BackupRecord.End(1, 0, 0, 1),
        )
        val oldOutput = ByteArrayOutputStream()
        codec.write(old.asSequence(), oldOutput)
        assertEquals(0, codec.validate(ByteArrayInputStream(oldOutput.toByteArray())) { }.usageCount)
    }
}
