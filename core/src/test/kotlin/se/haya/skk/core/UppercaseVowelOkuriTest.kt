...
class UppercaseVowelOkuriTest {
    ...
    @Test fun `重ねた子音が独自規則の接頭部なら完成まで送りとして保持する`() {
        // ss が即座に促音を出さない規則でも、未完成の s を語幹へ混入させません。
        for (hasShortRule in listOf(false, true)) {
            val rules = mutableListOf(RomajiRule("ka", "か"), RomajiRule("ssa", "しゃ"))
            if (hasShortRule) rules += RomajiRule("ss", "っ", "s")
            val lookups = mutableListOf<DictionaryQuery>()
            val engine = BasicSkkEngine(BasicSkkDictionary { query ->
                lookups += query
                listOf(DictionaryCandidate("候補"))
            }, RegistrationPolicy(), romanRuleSet = RomanRuleSet.compile(rules))

            engine.type("KasSa")

            assertEquals(listOf(DictionaryQuery("かs", "しゃ")), lookups)
            assertEquals("か", engine.state.reading)
            assertEquals("しゃ", engine.state.okuri)
            assertEquals("候補しゃ", engine.currentView.candidate?.committedText)
        }
    }
