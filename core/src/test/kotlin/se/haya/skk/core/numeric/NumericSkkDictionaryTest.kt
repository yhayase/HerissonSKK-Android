...
class NumericSkkDictionaryTest {
    ...
    @Test fun `数値予測の確定は展開結果を入力し元テンプレートを学習対象にする`() {
        val dictionary = facade(mapOf("だい#" to listOf(DictionaryCandidate("第#0"))))
        val engine = BasicSkkEngine(
            dictionary,
            RegistrationPolicy(sessionGeneration = 3),
            learningEnabled = true,
        )
        engine.dispatch(BasicSkkAction.SetTouchPrediction(true))
        engine.dispatch(BasicSkkAction.Text("Dai12", interpretCommands = false))

        assertEquals("第12", engine.currentView.prediction?.items?.single()?.committedText)
        val committed = engine.dispatch(BasicSkkAction.CommitPrediction())
        assertEquals("第12", committed.commit)
        val request = (committed.effects.single() as BasicSkkEffect.LearnCandidate).request
        assertEquals("第#0", request.candidate.text)
        assertEquals("第12", request.committedText)
        assertEquals("第#0", request.historyTarget?.templateText)
    }
