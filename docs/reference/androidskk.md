...
対象: [kachaya-ime/AndroidSKK](https://github.com/kachaya-ime/AndroidSKK)、コミット [`f5bf85c6d4482647531c608bc1037fc4206cea16`](https://github.com/kachaya-ime/AndroidSKK/tree/f5bf85c6d4482647531c608bc1037fc4206cea16)（2026-09-10）。ビルド設定の版は 2.1.0 です。導入済み APK とこのソースコミットの完全な一致は検証していません。

## 結論

対象コミットには幅広い機能があり、状態・モードの分離と Trie によるローマ字処理は再利用を検討できる構造です。一方、追跡できる自動回帰テストはなく、登録・学習・辞書形式・入力接続の失敗処理に具体的な問題がありました。この固定版を大きく拡張する場合は、それらの契約と試験を先に補う必要があります。

画面キーボード表示のような局所変更と、入力状態・辞書保存・Android 接続にまたがる変更では必要な検証が異なります。後者ではテスト導入、辞書処理の修正、コアと Android 接続の責務整理が判断材料になります。この調査は AndroidSKK の固定版を評価したもので、別製品との完成度比較ではありません。
...
- 22 項目の診断用アサーションが成立しました。これは製品の 22 テストが合格したという意味ではなく、正常系、問題の再現、仕様差、残存状態を確認した数です。[診断結果](androidskk-core-results.txt)を添付します。
