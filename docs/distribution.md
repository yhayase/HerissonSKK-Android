...
現在の実機確認用は `build/distributions/a1d748a/app-debug.apk` です。辞書をSQLite正本とする有限キャッシュへ変更し、起動・学習後の全件読み込みを廃止しました。未キャッシュ検索は非同期で処理します。SHA-256 は `5468041d3174d7e03168d80b188629b3658fb48c0f4dee562b270de94a0e451e` です。前回と同じ開発署名で上書き更新でき、辞書の再取り込みや保存形式の移行は不要です。最終APKに対する検証・性能比較を [作業記録](work-status.md) と配布ディレクトリの `manifest.json` に記録しています。
