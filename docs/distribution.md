...
現在の実機確認用は `build/distributions/81cb7ee/app-debug.apk` です。再帰登録・候補一覧・かな記号・編集キー・辞書一括適用の修正版です。SHA-256 は `ee7fdfeafa2ef70a502d9d50d54e7ecba1de4320ae85f4afd5e0dc7d53135e64` です。前回と同じ開発署名なのでアンインストールせず更新します。検証は [作業記録](work-status.md) に記載しています。入力設定は版3へ保存し、版1／2の読み込みと既存カスタムキーの保持に対応します。
