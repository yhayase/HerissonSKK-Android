...
製品は、依存コード・辞書の条件に問題がなければ非コピーレフトのpermissiveなOSSライセンスにします。具体的なライセンスは未決定です。S辞書の取得・同梱・再配布条件とAPKサイズ、著名辞書リストの取得元・条件を調査してから提供方法を決めます。ダウンロード方式だけでライセンス上の確認を省略しません。

ネットワーク取得は初回の追加要件です。以下の既存APKの権限検査は当時の構成の証拠です。実装時には通信に必要な権限と検査の期待値を見直し、ユーザー操作による取得・更新以外の無断更新がないことを検証します。現行の検査スクリプトは今回変更していません。
...
現在の実機確認用は `build/distributions/021c62f/app-debug.apk` です。「候補の全文」は選択中の候補または注釈が省略された場合だけ表示し、「ローカル辞書・限定試験辞書」の固定表示を削除しました。SHA-256 は `10b1b53b682d6c89e72b109dccbd0cbb9e0c7db4da169570bd92dcd9e6757a7c` です。前回と同じ開発署名で上書き更新できます。検証結果は [作業記録](work-status.md) と配布ディレクトリの `manifest.json` に記録しています。
