...
調査・決定日: 2026-09-23。所有者の決定により、本体の独自コード・独自文書には MIT ライセンスを採用します。[LICENSE](../LICENSE) と APK 用の同一本文を追加し、[適用範囲](../core/src/main/resources/META-INF/license-scope.txt)を明記しました。指定アプリアイコンは HerissonSKK のブランド識別として扱い、MIT の対象ではありません。使用条件は[アイコン使用条件](../core/src/main/resources/META-INF/icon-usage.txt)に定めます。
...
特に S 辞書は `InitialDictionaryInstaller` が初回に自動取得する製品側のデータであり、任意のユーザー持込みだけではありません。現行 release は限定試験辞書のフォールバックを無効にしており、公式辞書ファイルは APK に同梱していません。2026-09-23 にカタログ5辞書を実際に取得し、ヘッダーの著作権・条件・日時とハッシュを記録しました。初期設定から参照できる外部辞書の案内と GPLv2 本文をオフライン表示へ追加しました。[外部辞書の確認記録](reviews/dictionary-release-20260923.md)を参照します。端末ごとの将来の取得版を保存・固定したものではなく、辞書を同梱・転載する場合の対応ソース提供を代替する記録でもありません。
...
本体 LICENSE・第三者通知・アプリ内表示と、外部辞書5種類の取得元・条件・調査時点のハッシュ記録を整備しました。初期設定・カタログ・オフライン画面から条件を確認できます。最終公開成果物の収録検査は維持します。依存物や配布範囲を変更した場合は一覧と通知を更新します。
