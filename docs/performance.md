...
## 起動・打鍵計測を追加した時点の検証履歴（2026-09-16）

計測基盤を追加した初期段階では、測定用 APK と instrumentation APK のビルド、その時点の単体テスト 47 件、両アプリの Lint が成功しました。47 は当時の実行件数であり、要件から導いた必要数や現行の試験数ではありません。最新集計は [継続記録](work-status.md) に記載します。Python スクリプトの構文を確認しました。2026-09-16 にウィンドウと入力欄の準備待ちを追加した後、API 30 の専用エミュレーターで 1 回ずつのスモーク試験と、初回起動 30 回、起動済み 30 回、合成入力 1,000 回の通常測定が成功しました。通常測定のレポートは `test-editor/build/reports/performance/20260916T015010Z/` に生成しました。エミュレーターの数値は実機の評価には使用しません。
