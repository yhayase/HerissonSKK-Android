...
スクリプトは初回起動と起動済みをそれぞれ 30 回、準備入力後の打鍵を 1,000 回測定します。原因切り分け用に instrumentation を直接起動する場合は、`startup_samples` を 1〜30、`input_samples` を 1〜1,000 で指定し、短いスモーク試験にできます。省略時は通常の 30／1,000 回です。この引数は測定スクリプトの通常実行には使用しません。
