...
class PerformanceTest {
    ...
    private val target = "se.haya.skk.benchmark"
    ...
    @Test fun measure() {
        ...
        for ((name, samples) in listOf("cold" to cold, "warm" to warm)) {
            repeat(startupSamples) {
                ...
                if (name == "cold") {
                    shell("ime set $target/se.haya.skk.SkkInputMethodService")
                }
                withEditor { activity ->
    ...
    private fun awaitStatus(activity: InputTestActivity, editor: EditText) {
        ...
        val activities = diagnostic("dumpsys activity activities", listOf(
            "topResumedActivity=", "mResumedActivity:", "ResumedActivity:", "se.haya.skk.testeditor",
        ))
