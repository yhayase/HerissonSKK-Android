...
class SafDictionaryE2eTest {
    ...
    @Test fun replacePersonalThroughSafPublishesToImeAndExportsThenRestoresBackup() {
        ...
        val uniqueReadingSuffix = id.filter(Char::isLetterOrDigit).takeLast(12).map { character ->
            if (character.isDigit()) 'g' + (character - '0') else character
        }.joinToString("")
        val registrationReading = "zr$uniqueReadingSuffix"
        val deletionReading = "zz$uniqueReadingSuffix"
        ...
        try {
            ...
            ActivityScenario.launch(InputTestActivity::class.java).use { scenario ->
                ...
                awaitIme(registrationEditor)
                key(KeyEvent.KEYCODE_J, KeyEvent.META_CTRL_ON)
                // 自動導入された一般辞書と衝突しない英字見出しで、単語登録への遷移を検証します。
                type("/$registrationReading ")
                type("tango")
                // 単語登録の本文確定は登録状態の Enter として検証します。
            ...
            assertTrue("物理キー登録が個人辞書へ保存されません",
                exportedText.contains("$registrationReading /たんご/"))
    ...
    private fun openDictionarySettings() {
        openMainSettings()
        clickAppPreference("辞書の管理")
        awaitNode("辞書管理の読込が完了しません") { root ->
            root.takeIf { it.packageName?.toString() == SKK_PACKAGE }?.takeIf {
                val nodes = descendants(it)
                nodes.any { node -> node.text?.toString() == "辞書の管理" } &&
                    nodes.none { node -> node.viewIdResourceName == "$SKK_PACKAGE:id/settings_status" && node.isVisibleToUser } &&
                    nodes.any { node -> node.viewIdResourceName == "$SKK_PACKAGE:id/settings_save" && node.isEnabled }
            }
        }
    }
    ...
    private fun openLearningSettings() {
        openMainSettings()
        clickAppPreference("学習と補完")
        awaitTextVisible("学習と補完")
        awaitTextVisible(DYNAMIC_COMPLETION_LABEL)
    }
    ...
    private fun setDynamicCompletion(enabled: Boolean) {
        ...
        val changed = current.isChecked != enabled
        if (changed) {
        ...
        awaitNode("動的補完の設定が反映されません: $enabled") { root ->
            root.takeIf { it.packageName?.toString() == SKK_PACKAGE }
                ?.let { findPreferenceSwitch(it, DYNAMIC_COMPLETION_LABEL) }
                ?.takeIf { it.isChecked == enabled }
        }
        if (changed) {
            clickAppViewId("$SKK_PACKAGE:id/settings_save", "学習と補完の保存ボタンがありません")
            awaitTextVisible("設定")
        } else {
            openMainSettings()
        }
    ...
    private fun findPreferenceSwitch(root: AccessibilityNodeInfo, title: String): AccessibilityNodeInfo? {
        val titleNode = descendants(root).firstOrNull {
            it.isVisibleToUser && it.text?.toString() == title
        } ?: return null
        var row: AccessibilityNodeInfo? = titleNode
        while (row != null) {
            descendants(row).firstOrNull {
                it.isCheckable && it.isVisibleToUser
            }?.let { return it }
            row = row.parent
        }
        return null
    ...
    private fun exportPersonal(fileName: String) {
        clickAppPreference("個人辞書をファイルに保存")
        selectDownloads()
    ...
    private fun addSystemDictionary(fileName: String) {
        ...
        clickText("適用する")
        awaitTextVisible("変更はまだ保存されていません")
        if (findExactAppTextWithScroll(fileName) == null) {
    ...
    private fun restoreSuppressionIfPresent(sourceName: String, reading: String, candidate: String): Boolean {
        val detail = "$sourceName　読み: $reading　送り: なし"
        val row = findPreferenceWithScroll(candidate, detail) ?: return false
        assertTrue("対象の非表示候補を選べません", clickNodeOrParent(row))
    ...
    private fun removeSystemIfPresent(sourceName: String): Boolean {
        val action = findExactAppTextWithScroll("${sourceName}を削除")
            // 追加途中の失敗では辞書自体が存在しない場合があります。個人辞書の復元を妨げません。
            ?: return findExactAppTextWithScroll(sourceName) == null
        assertTrue("対象の追加辞書を選べません", clickNodeOrParent(action))
    ...
    private fun findPreferenceWithScroll(
        title: String,
        summary: String,
        ...
        while (SystemClock.uptimeMillis() < deadline) {
            automation.windows.asSequence().mapNotNull { it.root }
                ...
                .forEach { root ->
                    val anchor = descendants(root).firstOrNull { it.text?.toString() == title }
                    var row = anchor
                    while (row != null) {
                        val rowNodes = descendants(row)
                        if (rowNodes.any { it.text?.toString() == title } &&
                            rowNodes.any { it.text?.toString() == summary }) return row
                        row = row.parent
                    }
                }
            device.swipe(device.displayWidth / 2, device.displayHeight * 4 / 5,
    ...
    private fun findExactAppTextWithScroll(text: String): AccessibilityNodeInfo? {
        repeat(4) {
            device.swipe(device.displayWidth / 2, device.displayHeight / 5,
                device.displayWidth / 2, device.displayHeight * 4 / 5, 16)
            device.waitForIdle()
        ...
        while (SystemClock.uptimeMillis() < deadline) {
            automation.windows.asSequence().mapNotNull { it.root }
                ...
                .firstOrNull { it.isVisibleToUser && (it.text?.toString() == text || it.contentDescription?.toString() == text) }
                ?.let { return it }
            device.swipe(device.displayWidth / 2, device.displayHeight * 4 / 5,
                device.displayWidth / 2, device.displayHeight / 5, 16)
            device.waitForIdle()
    ...
    private fun clickSourceDialogItem(text: String) {
        awaitTextVisible("辞書の追加元")
        val deadline = SystemClock.uptimeMillis() + UI_TIMEOUT_MS
        while (SystemClock.uptimeMillis() < deadline) {
            val root = automation.windows.asSequence().mapNotNull { it.root }
                .firstOrNull { it.packageName?.toString() == SKK_PACKAGE &&
                    descendants(it).any { node -> node.text?.toString() == "辞書の追加元" } }
            val list = root?.let { descendants(it).firstOrNull { node ->
                node.className?.toString() == "android.widget.ListView" && node.isVisibleToUser
            } }
            if (list != null) {
                val item = descendants(list).firstOrNull { it.isVisibleToUser && it.text?.toString() == text }
                if (item != null && clickNodeOrParent(item)) return
                // 標準ダイアログの一覧が画面に収まらない場合も、一覧内をスクロールして選びます。
                list.performAction(AccessibilityNodeInfo.ACTION_SCROLL_FORWARD)
            }
            SystemClock.sleep(POLL_MS)
        }
        throw AssertionError("辞書の追加元を選択できません: $text")
    }

    private fun clickText(text: String) = clickAnyText(text)

    private fun clickAppPreference(text: String) {
        val node = findExactAppTextWithScroll(text)
            ?: throw AssertionError("設定項目がありません: $text")
        assertTrue("設定項目を操作できません: $text", clickNodeOrParent(node))
    }
    ...
    private fun clickAppViewId(id: String, message: String) {
    ...
    private fun selectedIme(): String = shell("settings get secure default_input_method").trim()
    ...
    /** 候補確定だけを行い、入力先の改行・送信設定には依存しません。 */
    private fun confirmCandidate() = key(KeyEvent.KEYCODE_J, KeyEvent.META_CTRL_ON)
    ...
    private companion object {
        ...
        const val DYNAMIC_COMPLETION_LABEL = "見出し語の補完"
