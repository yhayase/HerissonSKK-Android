...
初回公開はAPKを基本とします。既存の物理キー向け実装に加え、QWERTY画面キーボード、辞書のネットワーク導入・手動更新と初期S辞書、設定UI再設計、アプリ別Emacs編集オン／オフ、物理キー・タッチのIME切り替えを初回必須としました。これは未実装部分を含む計画です。範囲・優先順位は [ロードマップ](docs/roadmap.md)、Xiaomi Pad 8・Pixel 7と必須10アプリの受入は [検証計画](docs/validation-plan.md) を参照します。
